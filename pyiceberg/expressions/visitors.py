...
class _ProjectedColumnsEvaluator(BooleanExpressionVisitor[BooleanExpression]):
    ...
    Args:
      ...
      case_sensitive (bool): Whether to consider case when binding a reference to a field in a schema, defaults to True.
      projected_missing_fields(dict[int, Any]): Map of fields missing in file_schema, but present as partition values.
    ...
    def __init__(
        self, file_schema: Schema, projected_schema: Schema, case_sensitive: bool, projected_missing_fields: dict[int, Any]
    ) -> None:
        self.file_schema = file_schema
    ...
    def visit_bound_predicate(self, predicate: BoundPredicate[L]) -> BooleanExpression:
        ...
        if file_column_name is None and (field_id := predicate.term.ref().field.field_id) in self.projected_missing_fields:
            field = self.projected_schema.find_field(field_id)
            unbound_predicate: BooleanExpression
            if isinstance(predicate, BoundUnaryPredicate):
                unbound_predicate = predicate.as_unbound(field.name)
            elif isinstance(predicate, BoundLiteralPredicate):
                unbound_predicate = predicate.as_unbound(field.name, predicate.literal)
            elif isinstance(predicate, BoundSetPredicate):
                unbound_predicate = predicate.as_unbound(field.name, predicate.literals)
            else:
                raise ValueError(f"Unsupported predicate: {predicate}")
            schema = Schema(field)
            evaluator = expression_evaluator(schema, unbound_predicate, self.case_sensitive)
            if evaluator(Record(self.projected_missing_fields[field_id])):
                return AlwaysTrue()
            else:
                return AlwaysFalse()
...
def evaluate_projected_columns(
    ...
    case_sensitive: bool,
    projected_missing_fields: dict[int, Any],
) -> BooleanExpression:
