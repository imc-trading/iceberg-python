...
def test_merge_rejects_null_in_single_source_join_col(catalog: Catalog) -> None:
    """Source has NULL in the (only) join column. NULL has undefined
    equality semantics under SQL three-valued logic, so a merge with
    NULL keys would silently insert source NULL rows alongside any pre-
    existing target NULL rows, producing duplicate-NULLs that the merge
    "couldn't" deduplicate. Reject upfront with a clear, actionable
    error - matches upsert()'s effective behavior.
    """
    schema = Schema(
    ...
    with pytest.raises(ValueError, match=r"Join column\(s\) contain NULL value\(s\) in source: \['key'\]"):
...
def test_merge_rejects_null_in_one_of_composite_source_join_cols(catalog: Catalog) -> None:
    ...
    with pytest.raises(ValueError, match=r"NULL value\(s\) in source: \['key_a'\]"):
...
def test_merge_rejects_when_all_source_keys_null(catalog: Catalog) -> None:
    ...
    with pytest.raises(ValueError, match=r"NULL value\(s\) in source"):
        tbl.merge(src, join_cols=["key"])


def test_merge_rejects_null_in_target_join_col(catalog: Catalog) -> None:
    """Target has pre-existing NULL keys. Reject - matches the same
    contract as source-side: NULL is never a valid join key. The caller
    must clean target via delete() before merge() can run.

    The check is performed AFTER target_data is read (since it requires
    the data) but BEFORE the join, so we surface the bad data clearly
    instead of letting the anti-join silently keep the NULL rows.
    """
    schema = Schema(
        NestedField(1, "key", StringType(), required=False),
        NestedField(2, "value", IntegerType(), required=True),
    )
    ident = "default.merge_reject_target_null"
    _drop(catalog, ident)
    tbl = catalog.create_table(ident, schema=schema)
    ...
    with pytest.raises(ValueError, match=r"NULL value\(s\) in target: \['key'\]"):
        tbl.merge(src, join_cols=["key"])

    # Target unchanged: validation runs before any commit.
...
def test_merge_target_null_rejection_leaves_target_unchanged(catalog: Catalog) -> None:
    """Symmetric atomicity guarantee for target-side NULL rejection.
    Even though the validation runs after target_data is read, we must
    raise before any commit logic runs.
    """
    schema = Schema(
        NestedField(1, "key", StringType(), required=False),
        NestedField(2, "value", IntegerType(), required=True),
    )
    ident = "default.merge_atomic_target_null"
    _drop(catalog, ident)
    tbl = catalog.create_table(ident, schema=schema)
    ...
    with pytest.raises(ValueError, match="NULL value.*target"):
        tbl.merge(src, join_cols=["key"])
...
def test_merge_rejects_nan_in_source_float_join_key(catalog: Catalog) -> None:
    """Float join keys with NaN are rejected for the same reason NULLs
    are: ``NaN == NaN`` is false per IEEE 754, so a NaN key cannot
    participate in equality joins. Pin upfront rejection with a clear
    error - matches NULL handling semantically.
    """
    ...
    with pytest.raises(ValueError, match=r"NaN value\(s\) in source: \['key'\]"):
        tbl.merge(
            pa.Table.from_pylist(
...
def test_merge_rejects_nan_in_target_float_join_key(catalog: Catalog) -> None:
    """Symmetric NaN rejection on target side. If a candidate file
    contains NaN in the join col, raise with a clear error.
    """
    from pyiceberg.types import DoubleType
    ...
    ident = "default.merge_reject_target_nan"
    _drop(catalog, ident)
    tbl = catalog.create_table(ident, schema=schema)
    ...
    with pytest.raises(ValueError, match=r"NaN value\(s\) in target: \['key'\]"):
        tbl.merge(
            pa.Table.from_pylist(
